...
go-homeconnect2mqtt. It is modeled on the
...
gates: `make check`, which runs `go vet ./...`, a `gofumpt` formatting check, `golangci-lint run`,
the add-on changelog sync check, and the race-enabled test suite
(`go test -race -count=1 -timeout=120s ./...`). CI enforces the same steps on every pull request.
AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: go-homeconnect2mqtt is partly developed with AI assistance
