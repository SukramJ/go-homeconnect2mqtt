...
// mirrors hc_socket.ENCRYPT_DIRECTION / DECRYPT_DIRECTION.
const (
...
// DecodeKey decodes a url-safe base64 value without padding, matching the
// Python lib (which appends "===" and ignores surplus padding).
func DecodeKey(s string) ([]byte, error) {
	return base64.RawURLEncoding.DecodeString(s)
}

// deriveKeys runs the KDF: a single HMAC-SHA256 per label, no HKDF, no
// salt, no expand round. mirrors AesSocket.__init__.
...
// *CryptoError that obliges the caller to reconnect. mirrors
// AesSocket._receive.
